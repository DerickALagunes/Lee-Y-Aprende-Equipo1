...
public class login extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>error, como llegaste aqui?</h1>");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(validar.checkUsario(usuario,pass)){
            
            HttpSession sesion = request.getSession();
            Usuario user = validar.user(usuario, pass);
            sesion.setAttribute("user", true);
            sesion.setAttribute("info", user);
            
            if (validar.checkAdministrador(usuario, pass)) {
            ...
            RequestDispatcher rs = request.getRequestDispatcher("./bienvenido.jsp");
